...
class LineSource:
    ...
    def update_E(self):
        """Add the source to the electric field"""
        q = self.grid.time_steps_passed
        fwhm = 50 * self.grid.time_step
        max_step = 6 * int(fwhm / self.grid.time_step)
        # if pulse
        if self.pulse:
            #t1 = int(2 * pi / (self.frequency * self.hanning_dt / self.cycle))
            #if(q < max_step):
            vect = (self.profile /self.profile)* normalized_gaussian_pulse(q * self.grid.time_step, fwhm, fwhm * 3)
            #print(f"{vect}")
            #    #if q < t1:
            #    #    vect = self.profile * hanning(
            #    #        self.frequency, q * self.hanning_dt, self.cycle
            #    #    )
            #    #    #print(f"Profile good {t1}, {self.frequency}")
            #    #    #print(vect)
            #    #    #print(self.profile)
            #else:
            #print("Profile over")
